    ...
    for cl in CLUSTERING_MODEL:
        for nc in N_CLUSTERS:
            for r in ROUTED:
                ...
                salida_texto+= str(optimal_proba_cutoff)+'-'+str(cl)+'-'+str(tm)+'-'+str(nc)+'clusters-Routed'+str(r)+"=  accuracy: "+str(report2['accuracy']) + '\n'
                print(str(optimal_proba_cutoff)+'-'+str(cl)+'-'+str(tm)+'-'+str(nc)+'clusters-Routed'+str(r)+"=  accuracy: "+ str(report2['accuracy']) )#suma / 10
